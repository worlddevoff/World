...
export function PondTopBar() {
  const { pump, fish, holderCount, muted, setMuted } = usePond();
  const priceLabel = formatTokenPrice(pump.priceUsd);
  const hasPrice = pump.priceUsd != null && pump.priceUsd > 0;
  const biggest = fish.reduce((m, f) => Math.max(m, f.size), 0);

  return (
    <header className="z-30 flex items-center justify-between gap-3 border-b border-white/10 bg-[#0c4a6e] px-4 py-2.5 shadow-lg">
      <div className="flex min-w-0 items-center gap-3">
        <div className="flex items-center gap-2">
          ...
          <div className="leading-tight">
            ...
            <span className="hidden text-[10px] font-semibold text-sky-200/80 sm:block">
              The Pond
            </span>
          </div>
        </div>
      ...
      <div className="hidden items-center gap-5 md:flex">
        ...
        <div className="flex items-center gap-1.5 text-white/90">
          <Fish size={15} className="text-white/50" />
          <div className="leading-tight">
            <div className="text-[9px] uppercase text-white/45">Fish</div>
            <div className="text-sm font-bold">{fish.length}</div>
          </div>
        </div>
        <div className="flex items-center gap-1.5 text-white/90">
        ...
        {biggest > 0 && (
          <div className="rounded-full bg-sky-500/20 px-2.5 py-1 text-[11px] font-bold text-sky-100 ring-1 ring-sky-400/30">
      ...
      <div className="flex items-center gap-2">
        ...
        <div className="flex items-center gap-2 rounded-full border border-white/10 bg-black/25 px-3 py-1.5">
          <span className="text-xs font-bold text-sky-200">{TOKEN_TICKER}</span>
          <motion.span
            key={priceLabel}
